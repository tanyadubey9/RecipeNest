...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetchRecipes(); // Fetch default Indian recipes on load
    fetchCategories();
  }, []);
          ...
          recipes.map((recipe) => (
            <Link key={recipe.idMeal} href={`/meal/${recipe.idMeal}`}>
              <div className="bg-white p-4 shadow-md rounded-lg cursor-pointer hover:shadow-lg transition">
                ...
                <img src={recipe.strMealThumb} alt={recipe.strMeal} className="w-full 2xl:h-64 md:h-60 object-cover rounded-md mb-2" />
              </div>
            </Link>
          ))
        ) : (
...
//       {/* Sidebar */}
//       <aside className="relative w-64 bg-white p-6 shadow-md">
...
//       </div>
//         {session ? (
//           <h2 className="text-2xl font-semibold mb-5">Welcome, {session.user.name} 👋</h2>
//         ) : (
//           <h2 className="text-xl font-semibold mb-5">Welcome, Guest 👋</h2>
//         )}
//         <ul className="flex flex-col gap-2">
//           <li className="bg-blue-100 p-1 rounded-lg"><h2 className="text-xl font-semibold">Categories</h2></li>
//           <li><ul className="flex flex-col gap-1 p-1">
//             {categories.map((category) => (
//               <li
//                 key={category.idCategory}
//                 className="text-gray-700 hover:text-black cursor-pointer"
//                 onClick={() => fetchRecipesByCategory(category.strCategory)} // Fetch recipes on click
//               >
//                 {category.strCategory}
//               </li>
//             ))}
//           </ul></li>
//           <li className="bg-blue-100 p-1 rounded-lg"><Link href="/saved"><h2 className="text-xl font-semibold cursor-pointer hover:text-yellow-500">Save Recipe</h2></Link></li>
//         </ul>
...
//           <select
//             className="bg-[#d42287] text-white px-4 py-2 rounded-lg ml-2 hover:bg-[#9e0059] shadow-lg"
//             value={searchCategory}
//             onChange={(e) => setSearchCategory(e.target.value)}
//           >
//             <option value="country">By Country</option>
...
//             type="search"
//             placeholder={`Search by ${searchCategory}`}
//             className="border-2 p-2 rounded-lg w-1/3 ml-2 text-black"
//             value={searchTerm}
//             onChange={(e) => setSearchTerm(e.target.value)}
//           />

//           {/* Search Button */}
//           <button className="bg-[#d42287] text-white px-4 py-2 rounded-lg ml-2 hover:bg-[#9e0059]" onClick={fetchRecipes}>
//             Search
//           </button>
//         </div>
